...
Canonical copy. Sibling: space.heatmap/KKR_SpaceHeatmap_python/viewport_scene.py
...
from .overlay_model import TrajectoryOverlayModel
from .viewport_overlay import TrajectoryManipulator


class ViewportScene:
    """Manages a SceneView registered with the active viewport for 3D overlay rendering."""
    ...
    def __init__(self, ext_id: str):
        ...
        self.model = TrajectoryOverlayModel()
        ...
        with vp_win.get_frame(ext_id):
            ...
            with self._scene_view.scene:
                self._manipulator = TrajectoryManipulator(model=self.model)

        # CRITICAL: register with viewport API for camera tracking
        self._viewport_api.add_scene_view(self._scene_view)

    def destroy(self):
        """Unregister from viewport and clean up scene resources."""
