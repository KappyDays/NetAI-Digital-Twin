...
"""
Extension boilerplate for KKR.DynamicTracker.

Based on the lakehouse.proto template with viewport scene management added.

CRITICAL LIFECYCLE SEPARATION:
    - ViewportScene is created in on_startup() and destroyed in on_shutdown()
    - Panel window visibility (_on_window) does NOT touch the viewport scene
    - _on_window(visible=False) cleans up panel UI elements only
"""
...
class Extension(omni.ext.IExt):
    def on_startup(self, ext_id: str):
        ...
        # Viewport overlay — independent of panel window lifecycle
        self._viewport_scene = None
        try:
            self._viewport_scene = ViewportScene(ext_id)
            # Share the overlay model with UIBuilder so panel controls can update it
            self.ui_builder.set_overlay_model(self._viewport_scene.model)
        except Exception as e:
            print(f"[{EXTENSION_TITLE}] Viewport overlay setup failed: {e}")
    ...
    def on_shutdown(self):
        ...
        # Destroy viewport overlay FIRST
    ...
    def _on_window(self, visible):
        if self._window.visible:
            ...
            self.ui_builder.cleanup()
            # NOTE: Do NOT destroy viewport_scene here
