...
"""
Extension boilerplate for KKR.SpaceHeatmap.

CRITICAL LIFECYCLE SEPARATION:
    - ViewportScene is created in on_startup() and destroyed in on_shutdown().
    - _on_window(visible=False) cleans up panel UI only, NOT the viewport scene.
    - The viewport overlay persists independently of the panel window visibility.
"""


class Extension(omni.ext.IExt):
    def on_startup(self, ext_id: str):
        """Initialize extension, viewport scene, and UI elements."""

        self.ext_id = ext_id
        self._usd_context = omni.usd.get_context()
        self._viewport_scene = None

        # Create viewport scene (independent of panel window)
        try:
            self._viewport_scene = ViewportScene(ext_id)
        except RuntimeError as e:
            print(f"[KKR.SpaceHeatmap] Viewport scene init failed: {e}")
        ...
        action_registry.register_action(
            ext_id, self._action_name, self._menu_callback,
            description=f"Toggle {EXTENSION_TITLE} window",
        )
        self._menu_items = [
            MenuItemDescription(
                name=EXTENSION_TITLE, ticked=True,
        ...
        # Link overlay model to UIBuilder so panel controls can update the viewport
        if self._viewport_scene and self._viewport_scene.model:
            self.ui_builder.set_overlay_model(self._viewport_scene.model)
    ...
    def on_shutdown(self):
        ...
        # Destroy viewport scene (cleanup_cache called inside destroy)
    ...
    def _on_window(self, visible):
        """Panel visibility changed. Does NOT affect viewport scene."""
