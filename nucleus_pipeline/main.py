...
def raw_backup_main(args, backup_time=None):
    ...
    try:
        ...
        if download_success:
            ...
            # folder_path already set in Step 2
...
def _download_sublayers(local_usd_path, nucleus_url, dest_dir, script_dir, nucleus_token=""):
    """Download sublayer files referenced by the root USD layer.

    Runs in the main process where pxr is available (no DLL conflict).
    Each sublayer is downloaded via nucleus_client.py subprocess.
    """
    from pxr import Sdf

    layer = Sdf.Layer.FindOrOpen(local_usd_path)
    if not layer or not layer.subLayerPaths:
        return

    nucleus_dir = nucleus_url.rsplit("/", 1)[0]
    ...
    for sub_path in layer.subLayerPaths:
        if sub_path.startswith("omniverse://") or sub_path.startswith("http"):
            ...
            abs_url = f"{nucleus_dir}/{clean}"

        print(f"  Downloading sublayer: {sub_path}")
        cmd = [sys.executable, os.path.join(script_dir, "nucleus_client.py"),
               abs_url, dest_dir]
        env = os.environ.copy()
        if nucleus_token:
            env["NUCLEUS_TOKEN"] = nucleus_token
        result = subprocess.run(cmd, capture_output=True, text=True, env=env)
        if result.returncode == 0:
            print(f"  SubLayer downloaded: {sub_path}")
        else:
            print(f"  SubLayer download skipped: {sub_path}")


def entity_backup_main(args, backup_time=None):
    """Execute entity-level USD backup (Task 2)."""
    dest_dir = None  # for cleanup (Nucleus download)
    start_time = time.time()
    script_dir = os.path.dirname(os.path.abspath(__file__))

    try:
        print("=" * 60)
        print("Nucleus Pipeline v2: Entity Backup")
        print("=" * 60)
        ...
        # ── Step 3: Save to Iceberg ───────────────────────────────
        from lakehouse_client import generate_backup_time, send_backup
        if backup_time is None:
            backup_time = generate_backup_time()
